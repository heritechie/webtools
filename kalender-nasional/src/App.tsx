...
export default function App() {
	...
	return (
		<main className="mx-auto flex min-h-screen max-w-6xl flex-col gap-8 px-4 py-10 text-slate-50 sm:px-6 lg:px-8">
			<header className="flex flex-col gap-4">
				<div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
					...
					<label className="flex w-fit items-center gap-3 rounded-xl border border-slate-700 bg-surface-highlight px-4 py-2 text-sm text-slate-200">
						Pilih Tahun
						<select
							className="rounded-lg border border-slate-600 bg-surface-card px-3 py-1 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-accent-blue"
							value={activeYear}
			...
			<section className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1.2fr)]">
				<div className="flex flex-col gap-4">
					<div className="flex items-center justify-between">
						<h2 className="text-lg font-semibold text-white">Ringkasan Bulanan</h2>
								...
								<button
									...
									className={`group flex flex-col gap-3 rounded-2xl border p-4 text-left transition-all ${
										isActive
											...
											: 'border-slate-700/60 bg-surface-card hover:border-accent-blue/60 hover:shadow-md hover:shadow-accent-blue/10'
									}`}
								>
									<div className="flex items-center justify-between">
									...
									{upcoming ? (
										<div className="space-y-1">
											<p className="text-base font-semibold text-white">{upcoming.name}</p>
										...
										<p className="text-sm text-slate-400">Tidak ada libur nasional.</p>
									)}
								</button>
							);
						})}
				...
				<div className="flex flex-col gap-4 rounded-2xl border border-slate-700/60 bg-surface-card p-6 shadow-inner shadow-black/20">
					<header className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
						<div>
							<p className="text-sm uppercase tracking-[0.2em] text-slate-400">
								{getMonthLabel(calendar.month)} {activeYear}
							</p>
							<h2 className="text-2xl font-semibold text-white">Kalender Bulanan</h2>
						</div>
					...
					<div className="overflow-hidden rounded-xl border border-slate-700/60">
						<table className="w-full border-collapse">
							<thead className="bg-surface-highlight text-xs uppercase tracking-wider text-slate-300">
								<tr>
							...
							<tbody className="bg-surface-card text-sm text-slate-200">
								{calendar.weeks.map((week, weekIndex) => (
									<tr key={`week-${weekIndex}`} className="border-b border-slate-800/60">
										{week.map((cell) => (
											<td
												key={cell.date}
											...
											>
												<div
													className={`flex h-full flex-col gap-1 ${
														cell.holiday ? 'rounded-lg border border-accent-blue/40 bg-accent-blue/5 p-2' : ''
													}`}
												>
													<span
													...
													{cell.holiday ? (
														<span className="text-xs leading-snug text-accent-blue">{cell.holiday.name}</span>
													) : null}
												</div>
											</td>
										))}
					...
					<div className="space-y-3">
						...
						{calendar.holidays.length === 0 ? (
							<p className="rounded-lg border border-slate-800 bg-surface-highlight px-4 py-3 text-sm text-slate-400">
							...
							<ul className="space-y-3">
								{calendar.holidays.map((holiday) => (
									<li
										key={holiday.date}
										className="flex items-start justify-between rounded-lg border border-slate-800 bg-surface-highlight px-4 py-3"
									>
										<div>
											<p className="text-sm font-semibold text-white">{holiday.name}</p>
											<p className="text-xs text-slate-400">{holiday.description ?? 'Libur nasional'}</p>
										</div>
			...
			<footer className="rounded-2xl border border-slate-800 bg-surface-highlight px-4 py-3 text-xs text-slate-400">
